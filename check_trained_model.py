            ...
            if 'train_loss' in history and 'val_loss' in history:
                ...
                if len(val_losses) > 0:
                    ...
                    if train_losses[-1] < val_losses[-1]:
                        print(
                            f"  ⚠️  可能存在过拟合: 训练损失({train_losses[-1]:.6f}) < 验证损失({val_losses[-1]:.6f})")
            else:
                print("  ⚠️  训练历史数据不完整")
        else:
            print("  ⚠️  训练历史缺失")
        ...
        if 'model_state_dict' in checkpoint:
            ...
            for key, tensor in model_state.items():
                ...
                if 'lstm.weight_ih_l0' in key:
                    ...
                    hidden_dim = tensor.shape[0] // 4  # LSTM有4个门
...
def main():
    ...
    default_models = [
        ...
        os.path.join('models', 'best_bc_model.pth'),  # 旧路径，用于兼容
    ]
