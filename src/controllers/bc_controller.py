...
class BCController:
    ...
    def load_model(self):
        ...
        if is_heteroscedastic:
            output_dim = conf.get('output_dim', 8)  # 异方差：8维输出
        else:
            output_dim = conf.get('output_dim', 4)  # 同方差：4维输出

        model = BCNetwork(
            input_dim=conf.get('input_dim', 8),
            output_dim=output_dim,  # 修改点：使用配置的输出维度
    ...
    def update(self):
        ...
        # 3. 解析动作
        delta_pos = action[:3]  # 确保这里有 delta_pos
